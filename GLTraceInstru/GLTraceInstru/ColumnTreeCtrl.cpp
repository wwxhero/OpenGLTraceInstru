...
void CCustomTreeChildCtrl::CheckItemRecur(HTREEITEM hItem, BOOL check)
{
	SetCheck(hItem, check);
	HTREEITEM hItem_child = GetChildItem(hItem);
	while (NULL != hItem_child)
	{
		CheckItemRecur(hItem_child,check);
		hItem_child = GetNextSiblingItem(hItem_child);
	}
...
void CCustomTreeChildCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (CheckHit(point))
	{
		...
		if(hItem)
		{
			...
			SelectItem(hItem);
			BOOL check = GetCheck(hItem);
			CTreeCtrl::OnLButtonDown(nFlags, point);
			BOOL check_prime = GetCheck(hItem);
			if (m_bHierachicalSel
				&& check != check_prime)
				CheckItemRecur(hItem, check_prime);
		}
		else
...
void CCustomTreeChildCtrl::SetHierachicalSel(BOOL b)
{
	m_bHierachicalSel = b;
}
